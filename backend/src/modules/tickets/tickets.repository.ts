...
/**
 * Returns the total quantity of active (booked + paid) tickets a user holds
 * for a given event. Used to enforce the per-user cap across multiple bookings.
 */
export const getUserBookedCount = async (event_id: number, user_id: number): Promise<number> => {
  const result = await prisma.ticket.aggregate({
    where: { event_id, user_id, ticket_status: { in: [TicketStatus.booked, TicketStatus.paid] } },
    _sum: { quantity: true },
  });
  return result._sum.quantity ?? 0;
};
